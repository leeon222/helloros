            ...
            if result.returncode == 0:
                ...
                for line in output.splitlines():
                    if 'Type:' in line:
                        ...
                        # 将 ros2 msg type string 转换为 Python 类
                        # 例如: sensor_msgs/msg/LaserScan -> sensor_msgs.msg.LaserScan
                        if '/' in type_str:
                            pkg, msg = type_str.split('/')
                            msg_module = f'{pkg}.msg.{msg}'
                            # 动态导入
                            parts = msg_module.split('.')
                            module = __import__(msg_module, fromlist=[parts[-1]])
                            for part in parts[1:]:
